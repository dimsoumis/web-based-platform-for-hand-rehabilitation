...
[660, 50, "#0f0", 1]
	...
	function drawShapes(xT1, yT1, cT, cN, dests) {
...
ctx.fillText(cN, xT1 + numberAdjust, yT1 + 130);
		ctx.closePath();
...
// dest squares
	...
	ctx.beginPath();	
ctx.rect(dests[i][0], dests[i][1], 200, 200);
ctx.lineWidth = 10;
ctx.fillStyle = dests[i][2];
ctx.globalAlpha = 0.5;
ctx.fill();
ctx.closePath();
...
ctx.globalAlpha = 1;
ctx.font = "100px Arial";
ctx.fillText(dests[i][3], dests[i][0] + numberAdjust2, dests[i][1] + 130);
		ctx.closePath();
		}
...
ctx.closePath();
   ...
   drawShapes(xTetr1, yTetr1, ColorTetr1, currentNumber, destTetrs);
...
var countdown = setInterval(function() {
  ...
  if (seconds >= 600) {
	  ...
	  localStorage.setItem("ekatoneikosioktoRightTime", seconds);
	  localStorage.setItem("ekatoneikosioktoRightScore", currentNumber);
	    exerStarted = 0;
 ...
 location.reload()
	  ...
	  localStorage.setItem("ekatoneikosioktoRightScore", currentNumber);
			 ...
			 clearInterval(countdown);
			   		        ...
			   		        Swal.fire({
title: 'You did not manage to complete the exercise... \n Your score is ' + currentNumber + ' and you achieved it in ' + seconds + ' seconds.',
showDenyButton: true,
showCancelButton: true,
  confirmButtonText: `Play Again`,
  denyButtonText: `Go Back`,
...
/*dim custom code 2 start*/
 ...
 if (difAntiheirasDiktis < 0.05 && difAntiheirasMesos < 0.05 && thesiAntiheiraX > xTetr1 && thesiAntiheiraX < xTetr1 + 200
		   && thesiAntiheiraY > yTetr1 && thesiAntiheiraY < yTetr1 + 200 && grabbing == false) {
	 ...
	 if (thesiAntiheiraX > 1520 || thesiAntiheiraY > 780) {
...
camera.start();
		   ...
		   function  sendTetrUp() {
			   ...
			   var riseTetr = setInterval(function() {
	
				   if (yTetr1 <= 50) {

					   destTetrs.push([xTetr1, 50, ColorTetr1, currentNumber]);
		               xTetr1 = 1620;
					   yTetr1 = 100;
					   exerStarted = 1;
   ...
   drawShapes(xTetr1, yTetr1, ColorTetr1, currentNumber, destTetrs);
					   ...
					   clearInterval(riseTetr);	
					   	...
					   	yTetr1 = yTetr1 - 30;
				   	 	...
				   	 	for (let i = 0; i < destTetrs.length; i++) {
							
if (xTetr1 >= destTetrs[i][0] - 200 && xTetr1 <= destTetrs[i][0] + 200 && yTetr1 >= destTetrs[i][1] && yTetr1 <= destTetrs[i][1] + 200) {
	if (currentNumber != destTetrs[i][3]) {
		  destTetrs.push([xTetr1, destTetrs[i][1] + 200, ColorTetr1, currentNumber]);
		               xTetr1 = 1620;
					   yTetr1 = 100;
					   exerStarted = 1;
   ...
   drawShapes(xTetr1, yTetr1, ColorTetr1, currentNumber, destTetrs);
		...
		ColorTetr1 = "#f0f";
		} else if (currentNumber == 64) {
		ColorTetr1 = "#f80";
		} else if (currentNumber == 128) {
		ColorTetr1 = "#f08";
		}
		destTetrs[i][2] = ColorTetr1;
